...
class FacialRecognitionApp:
    ...
    def add_to_database(self):
        ...
        name = simpledialog.askstring("Face Name", "Enter a name for this face:")
        if not name:
            return
    ...
    def process_current_image(self, action_type):
        ...
        if self.current_image is None:
            messagebox.showinfo("Info", "No image to process. Please open an image or start the webcam first.")
            return
        ...
        if not isinstance(self.current_image, np.ndarray):
            try:
                ...
                messagebox.showerror("Error", f"Could not process the image: {str(e)}")
                return
        ...
        try:
            ...
            if action_type == "detect":
                ...
                # Face analysis with robust error handling
                try:
                    analysis = DeepFace.analyze(self.current_image, actions=['emotion', 'age', 'gender', 'race'], enforce_detection=False)
                    
                    result_img = self.current_image.copy()
                    self.log_result("Face Analysis Results:")
                    
                    # Handle different formats of results
                    if isinstance(analysis, list):
                        for i, face_data in enumerate(analysis):
                            self.process_face_analysis(result_img, face_data, i)
                    else:
                        # Single result (not in a list)
                        self.process_face_analysis(result_img, analysis, 0)
                    
                    self.display_image(result_img)
                    self.update_status("Analysis complete")
                except Exception as e:
                    print(f"Analysis error details: {e}")
                    messagebox.showerror("Analysis Error", f"Could not analyze face: {str(e)}")
                ...
                # Face verification
                try:
                    result = DeepFace.verify(self.reference_image, self.current_image, enforce_detection=False)
                    ...
                    messagebox.showerror("Verification Error", f"Could not verify faces: {str(e)}")
                ...
                try:
                    ...
                    messagebox.showerror("Recognition Error", f"Could not find faces in database: {str(e)}")
                    self.update_status("Database search failed")
    ...
    def process_face_analysis(self, image, face_data, index):
        """Helper function to extract and display face analysis results"""
        try:
            # Check if we have a dictionary with the expected structure
            if not isinstance(face_data, dict):
                self.log_result(f"Unexpected data format for face {index+1}")
                return
                
            # Extract face attributes with safe dictionary access
            # Handle different DeepFace result formats
            if 'emotion' in face_data and isinstance(face_data['emotion'], dict):
                # Nested structure - emotion is a dict with probabilities
            ...
            # Get gender
            if 'gender' in face_data and isinstance(face_data['gender'], dict):
                # Nested structure
                gender_dict = face_data['gender']
                gender = max(gender_dict.items(), key=lambda x: x[1])[0] if isinstance(gender_dict, dict) else 'unknown'
            else:
                gender = face_data.get('dominant_gender', 'unknown')
            
            # Get race/ethnicity
            if 'race' in face_data and isinstance(face_data['race'], dict):
                # Nested structure
                race_dict = face_data['race']
                race = max(race_dict.items(), key=lambda x: x[1])[0] if isinstance(race_dict, dict) else 'unknown'
            else:
                race = face_data.get('dominant_race', 'unknown')
            
            # Get face location
            x, y, w, h = 10, 10, 100, 100  # Default values
            
            # Try different possible locations for face coordinates
            if 'region' in face_data:
                # If region is in the result
                x = face_data['region'].get('x', 0)
                y = face_data['region'].get('y', 0)
                w = face_data['region'].get('w', 100)
                h = face_data['region'].get('h', 100)
            elif 'facial_area' in face_data:
                # If facial_area is in the result
                x = face_data['facial_area'].get('x', 0)
                y = face_data['facial_area'].get('y', 0)
                w = face_data['facial_area'].get('w', 100)
                h = face_data['facial_area'].get('h', 100)
            else:
                # Try to get from face detection
                try:
                    faces = DeepFace.extract_faces(self.current_image, detector_backend='opencv')
                    if index < len(faces):
                        x = faces[index]['facial_area'].get('x', 0)
                        y = faces[index]['facial_area'].get('y', 0)
                        w = faces[index]['facial_area'].get('w', 100)
                        h = faces[index]['facial_area'].get('h', 100)
                    else:
                        # Default position if face index is out of range
                        h, w = image.shape[:2]
                        x, y = 10, 30 + (index * 100)  # Stack text for multiple faces
                except:
            ...
            y_text = max(y, 70)  # Ensure enough space for text
            
            cv2.putText(image, f"Age: {age}", (x, y_text-40), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)
            cv2.putText(image, f"Gender: {gender}", (x, y_text-20), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)
            cv2.putText(image, f"Emotion: {emotion}", (x, y_text), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)
            
            # Generate results text
            face_result = f"Face {index+1} Analysis:\nAge: {age}\nGender: {gender}\nEmotion: {emotion}\nEthnicity: {race}"
            self.log_result(face_result)
            
        except Exception as e:
            print(f"Error in process_face_analysis: {e}")
            self.log_result(f"Error analyzing face {index+1}: {str(e)}")
                ...
                for i, filename in enumerate(image_files):
                    ...
                    try:
                        ...
                        try:
                            ...
                            for face in faces:
                                try:
                                    x = face['facial_area'].get('x', 0)
                                    y = face['facial_area'].get('y', 0)
                                    w = face['facial_area'].get('w', 0)
                                    h = face['facial_area'].get('h', 0)
                                    ...
                                    # Try to analyze the face
                                    try:
                                        # Create a smaller image with just the face for analysis
                                        face_img = img[y:y+h, x:x+w] if all([x, y, w, h]) else img
                                        
                                        # Only analyze if we have a valid face image
                                        if face_img.size > 0:
                                            ...
                                            # Handle both list and single result formats safely
                                            ...
                                            # Ensure text doesn't go off the top of image
                                            y_text = max(y, 70)
                                            cv2.putText(img, f"Age: {age}", (x, y_text-40), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)
                                            cv2.putText(img, f"Gender: {gender}", (x, y_text-20), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)
                                            cv2.putText(img, f"Emotion: {emotion}", (x, y_text), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)
                                    except Exception as e:
                                        print(f"Error analyzing face: {e}")
                                except Exception as e:
                                    print(f"Error processing face area: {e}")
                        except Exception as e:
